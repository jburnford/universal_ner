#!/usr/bin/env python3
"""
Process OLMoCR JSON output with Universal-NER to extract entities with toponym granularity.

Input: OLMoCR JSON file with structure: [{"id": "...", "text": "..."}]
Output: Enriched JSON with entity annotations including granular geographic entities
"""
import fire
import torch
import json
...
def extract_entities(generator, text, entity_types, max_new_tokens=256, use_enhanced_descriptions=False):
    """Extract entities of specified types from text with toponym granularity."""

    # Enhanced entity type descriptions for granular geographic extraction
    enhanced_descriptions = {
        ...
        # Granular geographic entity types
        ...
        # Fallback for mixed/unclear locations
...
def process_file(
    ...
    """
    Process an OLMoCR JSON file and extract entities with toponym granularity.

    Args:
        input_file: Path to OLMoCR JSON file
        output_file: Path for output JSON (default: input_file.ner.json)
        model_path: HuggingFace model path
        entity_types: Comma-separated entity types (supports: person, toponym, water_body, landform, administrative_region, route, organization, date, location)
    ...
    # Print statistics
    total_entities = sum(len(page.get('entities', {}).get(et, []))
                        for page in data
    ...
    for et in entity_types_list:
        count = sum(len(page.get('entities', {}).get(et, [])) for page in data)
        print(f"  {et}: {count}")
